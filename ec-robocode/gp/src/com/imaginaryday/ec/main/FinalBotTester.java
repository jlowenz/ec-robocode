...
public class FinalBotTester {
    ...
    final List<String> sampleBots = new ArrayList<String>();
    {
        ...
        sampleBots.add( "NanoAndrew" );
        sampleBots.add( "ChumbaWumba" );
        sampleBots.add( "AspidMovement" );
        sampleBots.add( "SilverSurfer" );
        sampleBots.add( "Freya" );
        sampleBots.add( "Wodan" );
        sampleBots.add( "Chiva" );
        sampleBots.add( "Merkava" );
        sampleBots.add( "Smoke" );
        sampleBots.add( "Nimrod" );
        sampleBots.add( "Smog" );
        sampleBots.add( "Virgin" );
        sampleBots.add( "Eve" );
        sampleBots.add( "Evader" );
        sampleBots.add( "Bandit" );
        sampleBots.add( "Wolverine" );
        sampleBots.add( "Sonda" );
        sampleBots.add( "Hawkwing" );
        sampleBots.add( "Terrible" );
        sampleBots.add( "PerceptBot" );
        sampleBots.add( "Predator" );
        sampleBots.add("UnderDark");
        sampleBots.add("Pasta");
        sampleBots.add("DustBunny");
        sampleBots.add("Ascendant");
        sampleBots.add("Hyperion");
        sampleBots.add("CrazyKitten");
        sampleBots.add("Predator-2");
        sampleBots.add("Movement");
        sampleBots.add("NanoGod");
        sampleBots.add("Ugluk");
        sampleBots.add("Grumpy");
        sampleBots.add("GloomyDark");
        sampleBots.add("Mako");
        sampleBots.add("Ali");
        sampleBots.add("DarkFinal");
        sampleBots.add("Fusion");
        sampleBots.add("RamRod");
        sampleBots.add("Mirror");
        sampleBots.add("MirrorNano");
        sampleBots.add("Squirtle");
        sampleBots.add("Charon");
        sampleBots.add("Pandora");
        sampleBots.add("Squirrel");
        sampleBots.add("TheArtOfWar");
        sampleBots.add("Wisdom");
    }


    public FinalBotTester(JavaSpace space, String filename, TransactionManager transactionManager) {

        this.space = space;
        this.transactionManager = transactionManager;
        try {
            summaryWriter = new FileWriter(System.getProperty("user.home") + System.getProperty("file.separator") + "FinalBotTestSummary.log", true);
            detailsWriter = new FileWriter(System.getProperty("user.home") + System.getProperty("file.separator") + "FinalBotTestDetails.log", true);
    ...
    private Map<Member, List<GPBattleResults>> collectResults(List<Member> population, int numBattles) {
        ...
        while (retrieved < numBattles) {
            ...
            try {
                while (res == null) {
                    if (takeCount >= FinalBotTester.MAX_TAKE_COUNT) {
                        resubmitTasks();
                        takeCount = 0;
                    }
    ...
    private void printDetails(Map<Member, List<GPBattleResults>> results) {
        ...
        for (Map.Entry<Member, List<GPBattleResults>> e : results.entrySet()) {
