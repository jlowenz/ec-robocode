...
public class GPAgent extends AdvancedRobot {
	...
	while (alive) {
	    ...
	    switch (r.second) {
		case LEFT: setTurnRadarLeftRadians(r.first); break;
		case RIGHT: setTurnRadarRightRadians(r.first); break;
	    }

	    // process turret directive
	    r = calculateTurn(getGunHeadingRadians(), turretDirection);
	    switch (r.second) {
		case LEFT: setTurnGunLeftRadians(r.first); break;
		case RIGHT: setTurnGunRightRadians(r.first); break;
	    }

	    // process heading/speed directive
	    r = calculateTurn(getHeadingRadians(), toAngle(movementVector));
	    switch (r.second) {
		case LEFT: setTurnLeftRadians(r.first); break;
		case RIGHT: setTurnRightRadians(r.first); break;
	    }
	    // set speed
	    double scale;
