...
public class GPFitnessCalc {
	...
	{
		return getScanAndSurviveFitness(numGenerations, robot, opponent);
	}
	...
	private static double getScanAndSurviveFitness(int numGenerations, RobotResults robot, RobotResults opponent) {
		...
		// bonus for survival
		double survialBonus = SURVIVAL_SCALE * robot.getEnergy() * 0.01 * robot.getBulletDamage(); // they start with 100,

	    double damageFitness = robot.getBulletDamage() + robot.getBulletDamageBonus() + robot.getRamDamage()
	            + robot.getRamDamageBonus() + accuracyBonus + scanEfficiencyBonus + survialBonus;
