...
*/
    ...
    public void evalRoundRobin(final EvolutionState state,
        ...
        if (state.evalthreads == 1)
            evalRoundRobinPopChunk(state, from[0], numinds[0], 0, individuals, prob);
        else {
     ...
     * @param prob
     */
    private void evalRoundRobinPopChunk(final EvolutionState state,
    ...
    private static class EncapsulatedIndividual {
    ...
    static class IndividualAndVictories {
        public Individual ind;
    ...
    static class IndComparator implements SortComparator {
        public boolean lt(Object a, Object b) { return ((Individual) a).fitness.betterThan(((Individual) b).fitness); }
        public boolean gt(Object a, Object b) { return ((Individual) b).fitness.betterThan(((Individual) a).fitness); }
    }

    static abstract class CompetitiveEvaluatorThread implements Runnable {
        public int numinds;
        public int from;
    ...
    static class RoundRobinCompetitiveEvaluatorThread extends CompetitiveEvaluatorThread {
        public synchronized void run() { me.evalRoundRobinPopChunk(state, from, numinds, threadnum, inds, p); }
    }

    static class NRandomOneWayCompetitiveEvaluatorThread extends CompetitiveEvaluatorThread {
        public synchronized void run() { me.evalNRandomOneWayPopChunk(state, from, numinds, threadnum, inds, p); }
    }

    static class NRandomTwoWayCompetitiveEvaluatorThread extends CompetitiveEvaluatorThread {
        public synchronized void run() { me.evalNRandomTwoWayPopChunk(state, from, numinds, threadnum, inds, p); }
    }
